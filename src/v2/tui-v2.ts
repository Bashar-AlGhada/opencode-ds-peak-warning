...
async function setupV2(context: Context): Promise<() => void> {
  ...
  const [guardSettings, setGuardSettings] = createSignal<GuardSettings>(
    loadGuard(kv, opts),
  );
  const [lastAck, setLastAck] = createSignal<number>(loadLastAck(kv));
  const [panelVisible, setPanelVisible] = createSignal<boolean>(
    loadPanelVisible(kv, opts),
  );
  ...
  const save = (next: TimeRange[]) => {
    ...
    const doc = makeCoverageDoc(
      migrateLegacyRanges(sanitizeRanges(next), WEEKDAY_DEFAULT_DAYS),
    );
    setCoverage(doc);
  ...
  const diagnosticLines = (): string[] => {
    ...
    lines.push(
      heard.length
        ? `Events: ${heard.map((t) => `${t} ${d.eventCounts[t]}`).join(" · ")}`
        : "Events: none yet",
    );
    ...
    if (silent.length) {
      ...
      for (const t of silent) {
        const parts = t.split(".");
        const key =
          parts[0] === "session" && parts[1] === "inbox"
            ? "session.inbox"
            : parts[0];
        groups.set(key, (groups.get(key) ?? 0) + 1);
      }
      lines.push(
        `Silent (${silent.length}): ${[...groups].map(([k, n]) => `${k}×${n}`).join(", ")}`,
      );
    }
    return lines;
  };
  ...
  const confirmPeak = () => {
    ack(Date.now());
    toast(
      `Peak acknowledged — quiet for ${formatCooldown(guardSettings().cooldownMs)}`,
      "warning",
    );
  };
